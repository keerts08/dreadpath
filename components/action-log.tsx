...
export default function ActionLog({ entries }: { entries: LogEntry[] }) {
  ...
  return (
    <div className="dread-scroll h-56 sm:h-64 overflow-y-auto border border-line bg-panel/60 p-3 text-sm leading-relaxed space-y-2">
      {entries.map((e) => (
        <p key={e.id} className={TONE_CLASS[e.tone]}>
          {e.tone === "whisper" && <span className="opacity-60">&ldquo;</span>}
          {e.text}
          {e.tone === "whisper" && <span className="opacity-60">&rdquo;</span>}
        </p>
      ))}
      <div ref={bottomRef} />
    </div>
